describe('User looking at the hotel page', () => {
    ...
    it('views custinfo page', () => {
        cy.get("h5[controlId='hotelname']")
            .should('contain', 'Grand Copthorne Waterfront');
        cy.get("h5[id='roomtype']")
            .should('contain', 'Beautiful Balcony');
        cy.contains('300').scrollIntoView()
            .should('be.visible');
    })
    ...
    it('interacts with all interactables and input is visible', () => {
        ...
        for (let i = 0; i < 3; i++) {
            cy.get("input#formBasicEmail.form-control").eq(i).scrollIntoView()
                .should('have.value', userInput[i]);
        }
        cy.get("#formBasicPhone").scrollIntoView()
            .should('have.value', "12345678");
        cy.get("#formBasicCountry").scrollIntoView()
            .should('have.value', "England");
        cy.get('#formBasicCheckbox')
            .should('be.checked');

        cy.wait(500);
        // Special Request
        // smoking pref
        cy.get("input#inline-radio-1.form-check-input").eq(0).click()
            .should('be.checked');
        cy.get("input#inline-radio-1.form-check-input").eq(1).click()
            .should('be.checked');
        // bed pref
        cy.get("input#inline-radio-2.form-check-input").eq(0).click()
            .should('be.checked');
        cy.get("input#inline-radio-2.form-check-input").eq(1).click()
            .should('be.checked');
        // any more req
        for (let i = 0; i < 4; i++) {
            cy.get("input#checkbox-2.form-check-input").eq(i).click()
                .should('be.checked');
        }
        // Personal req
        cy.get('textarea')
    ...
    it('fills up required info to book hotel', () => {
        ...
        cy.contains("Proceed to next step").scrollIntoView()
            .click({ force: true });
    ...
    it('did not fill up required info', () => {
        cy.contains("Proceed to next step").scrollIntoView()
            .click({force: true});
        cy.location('pathname').should('eq', '/custinfo')
        cy.contains('Please input valid first name').scrollIntoView()
            .should('be.visible');
        cy.contains('Please input valid last name').scrollIntoView()
            .should('be.visible');
        cy.contains('Please input valid email address').scrollIntoView()
            .should('be.visible');
        cy.contains('Please input valid country').scrollIntoView()
            .should('be.visible');
    })
